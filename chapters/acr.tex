...
\citetitle{LoufAmericanCultural2023}, that was previously published by the author of
this thesis with Bruno Gon\c{c}alves, Jos\'{e} J. Ramasco, David S\'{a}nchez and Jack
Grieve \cite{LoufAmericanCultural2023}. \\
...
example, in \cite{ZelinskyCulturalGeography1992} Wilbur Zelinsky
...
extensive range of cultural factors, the Raymond Gastil identified 13
...
factors has always been subjective.
A recent study interested in measuring cultural differences globally has leveraged a remarkable dataset to infer them, that features the interests of 2 billion Facebook users across tens of thousands of centres of interest as defined by the platform, but it still neither accounts for the intensity of users' interest nor for different weighting of interests \cite{ObradovichExpandingMeasurement2022}.
For example, religion and politics are undoubtedly
...
The research question will we address in this chapter is therefore: how can we infer
general American cultural regions in an objective way? In particular, how can we both
identify a complete or at least representative range of relevant cultural factors and
somehow combine these factors in so as to map American cultural regions? Defining such
regions does not mean that they do not contain internal variation or that they are
separated by hard borders --- culture is dynamic and complex and humans are highly
mobile --- but that we can find areas where the cultural practice and values of the
people who live within that region are more similar to each other than to those of
people who live outside that region.
...
sociolinguistics~\cite{NguyenComputationalSociolinguistics2016} --- as mentioned in
\cref{sec:methods_online_data} and done ourselves in the previous chapters --- and also
in particular to map patterns of dialect
...
corpora generated from microblogging platforms certainly present a number of biases that
we cited in \cref{sec:twitter_biases}. However, if cultural regions are real and
pervasive, then we should expect these regions to manifest themselves in any large
sample of everyday language that encompass a large proportion of the population, even if
the specific topics of interest vary across these different domains. Furthermore, as
highlighted in the discussion of \cref{sec:methods_twitter}, right now, Twitter is the
only variety of geotagged natural language data available in sufficient amounts to allow
reliable automatic analyses, and it serves as a perfect domain to apply our data-driven
approach for automatically mapping cultural regions.
...
\cref{fig:whole_data_clust}.

The hierarchical nature of the clustering is useful to see how regions are grouped
together at different levels of clustering, indicating which regions are closer
together. Importantly, applying hierarchical clustering to the principal dimensions of
variation of the data obtained through \ac{PCA} allows us to focus on the main regional
...
The hierarchy we obtain has all possible levels, from the highest with only two
clusters, to the lowest with as many clusters as counties. Not all these levels are
equally relevant though, so we need to select a number of clusters $n_{\text{clusters}}$
to consider. To do so, we first compute the silhouette score
\cite{RousseeuwSilhouettesGraphical1987} for different number of clusters, which is
defined as:
\begin{equation}
  s = \underset{c}{\text{mean}} \left(
    \frac{
      d_{c}^{\text{inter}} - d_{c}^{\text{intra}}
      }{
        \max (d_{c}^{\text{inter}}, d_{c}^{\text{intra}})
      } 
    \right),
\end{equation}
with $d_{c}^{\text{inter}}$ and $d_{c}^{\text{intra}}$ the mean intra- and inter-cluster
distances for county $c$. The higher it is, the more cohesive clusters for the level
corresponding to a number of clusters is. We show its values for up to 9 clusters in
\cref{fig:whole_data_clust}(b). Second, we draw a dendrogram, presented in
\cref{fig:whole_data_clust}(d). It is a tree that shows how clusters are merged together
and the distance between those that are merged, here on the x-axis. For the sake of
clarity we do not show the branches all the way to individual counties on the right, but
rather start at the merges relevant for the formation of $n_{\text{clusters}} = 5$.
...
regions are linked to clear and distinct topical patterns (see
\cref{ch:appendix_acr_words} for a more exhaustive list). We stress that these
characteristic words are automatically identified based on the quantitative analysis
presented above. Notably, for each cluster we see three basic types of lexical patterns. 
...
strongly associated with this region. This means that people in Texas and Oklahoma talk
more about place names in their own states than people in other regions we identified.
This is one type of regional topical patterns that our approach draws to identify
cultural regions.
...
diverse makeup of this region. This is characteristic of a situation of language contact
in multilingual regions, as the ones analysed in \cref{ch:multiling}. Although the
appearance of such code switching is intriguing, this is one aspect we have not covered
in that chapter, in which we made the assumption that languages were immutable entities. 
...
As shown by the dendrogram of \cref{fig:whole_data_clust}, it divides the South into two
regions, splitting Texas off from the rest of the Southeast, and splits the Midwest off
from the rest of the North, divided into discontinuous countryside/coastal regions,
rather than contiguous cultural regions. However, on the question of the number of
primary American cultural regions, we can only safely say that with our data and
methodology, at least 5 distinct regions can be discerned. We do not see it here, but we
still cannot discard recent theories that claim that America is fundamentally far more
...
\section{Discussion}
Overall, the analysis presented throughout this chapter has been able to identify regional patterns of lexical variation of
...
Clearly, the study presented in this chapter has only analysed one genre of American
English. The specific topical patterns on Twitter would not be exactly replicated in
other genres, especially given the communicative purpose and user base associated with
microblogging platforms. Nevertheless, assuming that American cultural regions are
important and pervasive forces, similar regional patterns should be reflected across all
popular genres. This issue could be further clarified when more richly annotated natural
language data becomes available in a near future. Our methods, however, will remain
valid for any such dataset. Crucially, we expect that our main idea of inferring
cultural regions and the topics defining them from people's speech will be applicable to
any big data resource with linguistic value.
