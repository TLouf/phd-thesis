...
for the very nature of the texts that were conserved --- books, legal texts mostly --- colloquial language is almost
...
across space, matching marginals obtained from the census using iterative proportional
...
\cite{BrownLanguageModels2020} Google's BERT \cite{DevlinBERTPretraining2019} and its
derivatives \cite{SanhDistilBERTDistilled2020}, or the BigScience workshop's BLOOM \cite{WorkshopBLOOM176BParameter2022}, with an increasing number of pre-trained
...
vector space (with for instance ``word2vec'' \cite{MikolovEfficientEstimation2013} and
``tok2vec'' \cite{AngelovTop2VecDistributed2020}, respectively). Although they are very
powerful tools to carry out some tasks, their training is very costly
...
to analyse social media speech, as found on Twitter for instance. Some works have
strived to normalise informal texts to be able to use such tools, but doing so throws
...
to determine that the model only allowed stable coexistence of two languages for $a > 1$
\cite{VazquezAgentBased2010}. Many other models of the same kind have been proposed to
understand language shift, extending previous formulations to consider additional
...
% macro \cite{HanskiMetapopulationDynamics1998}??
