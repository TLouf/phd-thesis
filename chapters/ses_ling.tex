...
\begin{figure}
\centering
  \includegraphics[width=\textwidth]{twitter_pop.png}
...
We denote $t_{u, c}$ the number of trips made by
...
\begin{equation}
  ...
  M_{i, j} = \frac{
      \sum_{c^* \in S_i} \sum_{u \in c^*} \sum_{c \in S_j} t_{u, c}
    }{
      \sum_{c^* \in S_i} \sum_{u \in c^*} \sum_{k = 1}^{n_S} \sum_{c \in S_k} t_{u, c}
    },
\end{equation}
...
\centering
  ...
  This example was selected for having one of the lowest correlations between x and y axis values, which are shown for other values of $q_1$ and $l_v$ for $q_2 = 0.5$ in (b), and of $q_1$ and $q_2$ for $l_v = 0.55$ in (c).}
