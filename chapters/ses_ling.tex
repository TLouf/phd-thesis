...
This system of equation lends itself to mathematical analysis, in particular of its
fixed points and their stability. Two fixed points can be trivially found from a quick
look at the equations: they are $(p_1, p_2) = (0, 1)$ and $(1, 0)$. As in
\cref{ch:multiling}, we compute the eigenvalues of the Jacobian of the system evaluated
at the fixed points, and are thus able to say that a fixed point is stable if and only
if all the corresponding eigenvalues have a strictly negative real part. For $M = 0.2$
$q_2= 0.5$, $q_1 = 0.7$, and $l_v \in \{ 0.5, 0.6, 0.7\}$, we show in
...
\Cref{fig:mean_field_analytic_coex_pos} shows how this stable solution moves in the
