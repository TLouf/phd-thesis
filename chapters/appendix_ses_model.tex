...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
Let us consider two varieties $1$ and $2$. This could be the use of standard
language ($1$ means they do, $2$ means they do not). Now let us introduce an
intrinsic prestige $l_v$ for the variety $2$, such that:
\begin{equation}
    \begin{aligned}
...
c_t = c_j)$ the probability to pick an individual who used variety $v$ at $t-1$ who is at
...
further, which summarise the state of the system at the previous step. Also, since
...
N_{s_k}}$. The final term in the product above is related to the mobility of the different SES
...
\begin{equation}
    ...
    \begin{aligned}
        P(v_{t-1} = 2 \mid c = c_1)
            = (1 - m_2) (1 - p_1) + m_2 p_2
        \\
        P(v_{t-1} = 2 \mid c = c_2)
            = m_1 (1 - p_1) + (1 - m_1) p_2
        \\
        P(v_{t-1} = 1 \mid c = c_1)
            = (1 - m_2) p_1 + m_2 (1 - p_2)
        \\
        P(v_{t-1} = 1 \mid c = c_2)
            = m_1 p_1 + (1 - m_1) (1 - p_2)
    \end{aligned}
\end{equation}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
Finally, inserting \eqref{eq:first_trans_probs} into \eqref{eq:trans_probs_cell_decomp}, we get:
\begin{equation}
    ...
    \begin{aligned}
        P(1 \rightarrow 2 \mid s = s_1)
            &= l_v (1 - q_1)
            \begin{aligned}[t]
            &[(1 - M_1) P(v_{t-1} = 2 \mid c = c_1)
            \\
            & + M_1 P(v_{t-1} = 2 \mid c = c_2)]
            \end{aligned}
        \\
        P(1 \rightarrow 2 \mid s = s_2)
            &= l_v q_2
            \begin{aligned}[t]
                &[M_2 P(v_{t-1} = 2 \mid c = c_1)
                \\
                &+ (1 - M_2) P(v_{t-1} = 2 \mid c = c_2)]
            \end{aligned}
        \\
        P(2 \rightarrow 1 \mid s = s_1)
            &= (1 - l_v) q_1
            \begin{aligned}[t]
                &[(1 - M_1) P(v_{t-1} = 1 \mid c = c_1)
                \\
                &+ M_1 P(v_{t-1} = 1 \mid c = c_2)]
            \end{aligned}
        \\
        P(2 \rightarrow 1 \mid s = s_2)
            &= (1 - l_v) (1 - q_2)
            \begin{aligned}[t]
                &[M_2 P(v_{t-1} = 1 \mid c = c_1)
                \\
                &+ (1 - M_2) P(v_{t-1} = 1 \mid c = c_2)]
            \end{aligned}
    \end{aligned}
\end{equation}
...
\begin{equation}
    ...
    \end{aligned}
\end{equation}
with $M^* = 2 M (1 - M)$. The result presented in \cref{eq:ses_ling_time_evol_eq_mob} then follows.
